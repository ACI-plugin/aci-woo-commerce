...
class WC_Ignite {
	 ...
	 * @return array Modified array of data store classes.
	 */
	...
	public function wc_ignite_dependencies() {
		...
		require_once WC_IGNITE_PLUGIN_FILE_PATH . 'ignite/includes/traits/wc-ignite-initialize-trait.php';
		...
		require_once WC_IGNITE_PLUGIN_FILE_PATH . 'ignite/includes/settings/admin/class-wc-ignite-custom-fields-settings.php';
		...
		$setting_classes = apply_filters(
			...
			array(
				...
				'custom_settings' => 'WC_Ignite_Custom_Fields_Settings',
			)
		);
	...
	public function get_payment_gateways() {
		 ...
		 * @since 1.0.0
		 */
		if ( is_wc_endpoint_url( 'add-payment-method' ) && ! ( apply_filters( 'ignite_enable_add_payment_method_link', false ) ) ) {
			return array();
		}
		/**
		 * Filter to enable/disable payment methods on My Account page.
		 *
		 * @param bool true/false Default is true.
		 *
		 * @since 1.0.0
		 */
	 ...
	 *
	 * Note: HPOS compatibility is now declared in the ACI class to avoid duplicate declarations
	 * since this is now a combined plugin with a single entry point.
	 */
	public function woo_ignite_plugins_loaded() {
		// HPOS compatibility declaration moved to ACI class
	}
}
