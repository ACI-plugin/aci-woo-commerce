...
class WC_Payment_Gateway_Ignite_CC_Offsite extends WC_Payment_Gateway_Ignite {
	...
	public function __construct() {
		...
		$this->method_title = __( 'Ignite Redirect Payment Settings', 'woocommerce' );
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			$ignite_params          = $this->prepare_initialize_request();
			...
			$params                  = array(
				'cartTotalAmount'       => $this->get_formatted_price( $cart_total_amount ),
				'currency'              => $currency,
			...
			$response                = $this->gateway->initialize->create( $params );
			...
			if ( ! empty( $json_res['redirectUrl'] ) ) {
				...
				$order->save_meta_data();
				$order->set_transaction_id( $json_res['transactionId'] );
				$order->save();
