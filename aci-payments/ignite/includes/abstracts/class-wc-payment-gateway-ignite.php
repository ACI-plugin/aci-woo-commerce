...
abstract class WC_Payment_Gateway_Ignite extends WC_Payment_Gateway {
	...
	use WC_Ignite_Initialize_Trait;
	...
	public function is_available() {
		...
		if ( ! $this->is_ignite_enabled() || ( WC()->cart && 0 < $this->get_order_total() && 0 < $this->max_amount && $this->max_amount < $this->get_order_total() ) ) {
			$is_available = false;
		}
		return $is_available;
	}
	...
	/**
	 * Check whether the wehook mode is enabled or not
	 *
	 * @return bool
	 */
	public function is_webhook_mode_enabled() {
		$settings = $this->get_ignite_setting();
		$mode     = $this->get_api_mode();
		return ( isset( $settings['webhook_mode'] ) && 'yes' === $settings['webhook_mode'] && 'test' === $mode );
	}
}
