 ...
 */
trait WC_Fc_Initialize_Trait {

	/**
	 * Prepare Ignite request data.
	 *
	 ...
	 */
	public function prepare_fc_request( $gateway_key, $cart_total_amount ) {
		...
		return $this->remove_null_value( array_merge( $billing_array, $shipping_array, $cart_item_array, $custom_param_array, $customer_array ) );
	}
	...
	public function prepare_cart_item( $order, $gateway_key, $cart_total_amount ) {
		...
		foreach ( $line_items as $line_item ) {
			...
			$cart_item_array[] = array(
				...
				$this->key_fc_payment_currency    => $order->get_currency(),
	...
	public function prepare_custom_parameters() {
		...
		$custom_parameters = array(
			...
			$this->key_module_version => WC_IGNITE_VERSION,
		);
