...
class WC_Ignite_Rest_Controller {
	...
	public function webhook( WP_REST_Request $request ) {
		...
		$logger->debug( wp_json_encode( $json_payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ), array( 'source' => 'Ignite-Webhook-Request' ) );
		if ( ! $json_payload ) {
		...
		try {
			...
			$payload_array['Timestamp'] = time();

			// Encode back to JSON.
			$updated_json_payload = wp_json_encode( $payload_array, JSON_PRETTY_PRINT );
